...
async function renderGoals() {
  ...
  document.getElementById('goalsContainer').innerHTML = state.goals.map(g => {
    ...
    return `
      <div class="card animate-fade" style="margin-bottom:12px">
        ...
        <div style="display:flex;justify-content:space-between;font-size:0.85rem;color:var(--text-secondary);margin-bottom:4px">
          <span>${fmtMoney(current)} / ${fmtMoney(g.target_amount)}</span>
          <span>${pct}%</span>
        </div>
        <div class="progress-bar"><div class="progress-fill" style="width:${pct}%"></div></div>
        <div style="font-size:0.8rem;color:var(--text-muted);margin-top:8px">${projectionText}</div>
      </div>`;
  }).join('');
}
