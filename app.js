...
const PHASES = {
  vie: ['Idée','Recherche','En cours','Lancé','Terminé','Abandonné'],
  argent: ['Idée','Recherche','En cours','Acquis','Abandonné']
};
...
async function loadAllData() {
  ...
  state.goals = await sbSelect('goals', { order: { column: 'created_at', asc: false } });
...
function toggleSidebar() {
  ...
  document.querySelector('.mobile-tabbar')?.classList.toggle('hidden');
}
...
function navigateTo(view, skipHistory) {
  ...
  const tabGroup = { accueil:'accueil', filrouge:'tasks', agenda:'tasks', tasks:'tasks', menus:'menus', recipes:'menus', projects:'wallet', wallet:'wallet' };
  document.querySelectorAll('.mobile-tab').forEach(t => t.classList.toggle('active', t.dataset.view === tabGroup[view]));
  closeSidebar();
...
function renderFilRouge() {
  ...
  // Timeline : événements + tâches du jour qui ont une heure précise, triés ensemble
  const todaysEvents = state.events.filter(e => {
    const d = new Date(e.date); d.setHours(0,0,0,0);
    return d.getTime() === today.getTime();
  }).map(e => ({ kind:'event', time: e.time, title: e.title, meta: [e.lieu, e.numero].filter(Boolean).join(' · ') }));

  const todaysTimedTasks = state.tasks.filter(t => !t.done && isToday(t.due) && t.due_time)
    .map(t => ({ kind:'task', time: t.due_time, title: t.title, meta: t.prio==='urgent'?'🔴 Urgent':(t.prio==='important'?'🟠 Important':'') }));

  const timelineItems = [...todaysEvents, ...todaysTimedTasks].sort((a,b) => (a.time||'99:99').localeCompare(b.time||'99:99'));

  const timelineHTML = timelineItems.map(item => `
    <div class="timeline-item ${item.kind} animate-fade">
      <div class="timeline-time">${item.time || 'Toute la journée'}</div>
      <div class="timeline-title">${item.kind==='task' ? '✅ ' : ''}${escapeHtml(item.title)}</div>
      <div style="font-size:0.8rem;color:var(--text-muted)">${escapeHtml(item.meta || '')}</div>
    </div>
  ...
  document.getElementById('nextActionsContainer').innerHTML = activeProjects.slice(0,5).map(p => `
    <div class="alert-item info animate-fade" style="cursor:pointer" onclick="navigateTo('projects')">
      <span class="badge badge-${p.cat}">${p.cat==='vie'?'Vie':'Argent'}</span>
...
function updateTaskFields() {
  ...
  if (tmpl === 'rdv') extra = `
    <div class="form-group">
      ...
      <select id="taskSpecialite" onchange="document.getElementById('taskSpecialiteAutre').style.display=this.value==='autre'?'block':'none'">
        ...
        <option value="administratif">Administratif (préfecture, CAF, impôts...)</option>
        <option value="autre">Autre</option>
      </select>
    </div>
    <div class="form-group" id="taskSpecialiteAutre" style="display:none"><label>Précise</label><input type="text" id="taskSpecialiteAutreText"></div>
    <div class="form-group"><label>Organisme / Contact</label><input type="text" id="taskContact"></div>
    <div class="form-group"><label>Lien de réservation</label><input type="url" id="taskLien"></div>`;
...
const AISLES = [
  { name: 'Fruits & Légumes', emoji: '🥦', keywords: ['pomme','poire','banane','orange','clementine','mandarine','citron','pamplemousse','salade','laitue','roquette','tomate','carotte','patate','oignon','echalote','ail','courgette','aubergine','poivron','concombre','avocat','fraise','framboise','myrtille','raisin','kiwi','ananas','mangue','melon','pasteque','peche','abricot','prune','cerise','figue','grenade','brocoli','chou','champignon','epinard','poireau','haricot vert','petits pois','celeri','navet','betterave','potiron','courge','radis','fenouil','mais','persil','coriandre','basilic','menthe','thym','gingembre','legume','fruit','herbe'] },
  { name: 'Boulangerie', emoji: '🥖', keywords: ['pain','baguette','croissant','brioche','biscotte','viennoiserie','tortilla','wrap','pita','burger'] },
  { name: 'Produits laitiers & Œufs', emoji: '🥛', keywords: ['lait','yaourt','yoghourt','fromage','beurre','creme','oeuf','petits suisses','emmental','comte','camembert','chevre','mozzarella','parmesan','roquefort','feta','ricotta','mascarpone','skyr'] },
  { name: 'Viande & Poisson', emoji: '🍗', keywords: ['poulet','boeuf','porc','jambon','saucisse','chipolata','merguez','steak','poisson','saumon','thon','cabillaud','colin','truite','sardine','maquereau','crevette','moule','calamar','surimi','dinde','agneau','veau','canard','lardon','bacon','roti','escalope','cuisse','filet','cote'] },
  { name: 'Épicerie salée', emoji: '🍝', keywords: ['pates','spaghetti','penne','tagliatelle','lasagne','riz','quinoa','boulgour','semoule','couscous','farine','fecule','levure','huile','sel','poivre','paprika','curry','cumin','curcuma','herbes de provence','sauce','moutarde','ketchup','mayonnaise','vinaigre','conserve','lentille','haricot','pois chiche','soupe','bouillon','cornichon','olive','capre','tahini','houmous','concentre','passata'] },
  { name: 'Épicerie sucrée', emoji: '🍫', keywords: ['sucre','cassonade','chocolat','gateau','biscuit','confiture','compote','miel','cereales','muesli','granola','barre','bonbon','chips','cacahuete','amande','noix','pistache','raisins secs','datte','pate a tartiner','aperitif'] },
  { name: 'Boissons', emoji: '🥤', keywords: ['eau','jus','soda','coca','limonade','sirop','cafe','the','infusion','vin','biere','champagne','boisson'] },
  { name: 'Surgelés', emoji: '🧊', keywords: ['surgele','glace','sorbet','frite','nugget','pizza','pane'] },
  { name: 'Hygiène & Entretien', emoji: '🧻', keywords: ['savon','shampoing','dentifrice','brosse a dent','papier toilette','essuie-tout','mouchoir','lessive','adoucissant','eponge','vaisselle','deodorant','rasoir','mousse a raser','coton','creme hydratante','protection','sac poubelle','sacs poubelle','nettoyant','javel','gel douche','aluminium','film alimentaire','papier cuisson','tablette'] },
  { name: 'Bébé', emoji: '🍼', keywords: ['couche','lingette','lait infantile','petit pot','petits pots','cereales bebe','bebe'] },
  { name: 'Animaux', emoji: '🐾', keywords: ['croquette','patee','litiere','chien','chat'] }
];
// Normalise le texte pour la recherche : "Œufs" trouvable en tapant "oeu", "Café" en tapant "cafe"
...
function getArticleMeta(name) {
  ...
  for (const aisle of AISLES) {
    if (aisle.keywords.some(k => n.includes(normalizeSearch(k)))) return aisle;
  }
...
const PRODUCT_DB = [
  // --- Fruits ---
  'Pommes', 'Poires', 'Bananes', 'Oranges', 'Clémentines', 'Citrons', 'Citrons verts', 'Pamplemousse',
  'Fraises', 'Framboises', 'Myrtilles', 'Raisin', 'Kiwis', 'Ananas', 'Mangue', 'Melon', 'Pastèque',
  'Pêches', 'Abricots', 'Prunes', 'Cerises', 'Figues', 'Avocat', 'Grenade',
  // --- Légumes ---
  'Tomates', 'Tomates cerises', 'Salade', 'Laitue', 'Roquette', 'Épinards', 'Carottes', 'Pommes de terre',
  'Oignons', 'Oignons rouges', 'Échalotes', 'Ail', 'Courgettes', 'Aubergines', 'Poivrons', 'Concombre',
  'Brocoli', 'Chou-fleur', 'Chou', 'Poireaux', 'Haricots verts', 'Petits pois', 'Champignons',
  'Céleri', 'Navets', 'Betteraves', 'Potiron', 'Patate douce', 'Radis', 'Fenouil', 'Maïs',
  'Persil', 'Coriandre', 'Basilic', 'Menthe', 'Thym', 'Gingembre',
  // --- Boulangerie ---
  'Pain', 'Baguette', 'Pain de mie', 'Pain complet', 'Pain aux céréales', 'Croissants', 'Pains au chocolat',
  'Brioche', 'Biscottes', 'Pain burger', 'Tortillas', 'Pain pita', 'Wraps',
  // --- Produits laitiers & Œufs ---
  'Œufs', 'Œufs (pack de 6)', 'Œufs (pack de 12)', 'Lait', 'Lait demi-écrémé 1L', 'Lait entier 1L',
  'Lait sans lactose', 'Lait d\'amande', 'Lait de soja', 'Lait d\'avoine', 'Beurre', 'Beurre doux',
  'Beurre demi-sel', 'Crème fraîche', 'Crème liquide', 'Yaourts nature', 'Yaourts aux fruits',
  'Yaourt grec', 'Fromage blanc', 'Petits suisses', 'Fromage râpé', 'Emmental', 'Comté', 'Camembert',
  'Chèvre', 'Mozzarella', 'Parmesan', 'Roquefort', 'Feta', 'Ricotta', 'Mascarpone',
  // --- Viande & Poisson ---
  'Poulet', 'Filets de poulet', 'Cuisses de poulet', 'Escalopes de dinde', 'Bœuf haché', 'Steaks hachés',
  'Steak', 'Rôti de bœuf', 'Côtes de porc', 'Filet mignon', 'Saucisses', 'Merguez', 'Chipolatas',
  'Lardons', 'Jambon blanc', 'Jambon cru', 'Bacon', 'Agneau', 'Veau', 'Canard',
  'Saumon', 'Saumon fumé', 'Cabillaud', 'Colin', 'Truite', 'Thon en boîte', 'Sardines', 'Maquereaux',
  'Crevettes', 'Moules', 'Calamars', 'Surimi',
  // --- Épicerie salée ---
  'Pâtes', 'Spaghetti', 'Penne', 'Tagliatelles', 'Lasagnes', 'Riz', 'Riz basmati', 'Riz complet',
  'Quinoa', 'Boulgour', 'Semoule', 'Couscous', 'Lentilles', 'Pois chiches', 'Haricots rouges',
  'Haricots blancs', 'Farine', 'Fécule de maïs', 'Levure', 'Huile d\'olive', 'Huile de tournesol',
  'Vinaigre', 'Vinaigre balsamique', 'Sel', 'Poivre', 'Paprika', 'Curry', 'Cumin', 'Curcuma',
  'Herbes de Provence', 'Moutarde', 'Ketchup', 'Mayonnaise', 'Sauce soja', 'Sauce tomate',
  'Concentré de tomate', 'Passata', 'Bouillon cube', 'Soupe', 'Conserve de thon', 'Cornichons',
  'Olives', 'Câpres', 'Miel', 'Tahini', 'Houmous',
  // --- Épicerie sucrée ---
  'Sucre', 'Sucre vanillé', 'Cassonade', 'Chocolat noir', 'Chocolat au lait', 'Chocolat pâtissier',
  'Pâte à tartiner', 'Confiture', 'Compote', 'Biscuits', 'Gâteaux', 'Céréales', 'Muesli', 'Granola',
  'Barres de céréales', 'Bonbons', 'Chips', 'Gâteaux apéritifs', 'Cacahuètes', 'Amandes', 'Noix',
  'Noix de cajou', 'Pistaches', 'Raisins secs', 'Dattes',
  // --- Boissons ---
  'Eau', 'Eau (pack de 6)', 'Eau pétillante', 'Jus d\'orange', 'Jus de pomme', 'Jus multifruits',
  'Soda', 'Coca-Cola', 'Limonade', 'Sirop', 'Café', 'Café moulu', 'Capsules de café', 'Thé',
  'Thé vert', 'Infusion', 'Chocolat en poudre', 'Vin rouge', 'Vin blanc', 'Bière', 'Champagne',
  // --- Surgelés ---
  'Glace', 'Sorbet', 'Frites surgelées', 'Légumes surgelés', 'Poisson pané', 'Nuggets',
  'Pizza surgelée', 'Épinards surgelés', 'Fruits rouges surgelés', 'Pain surgelé',
  // --- Hygiène & Entretien ---
  'Papier toilette', 'Essuie-tout', 'Mouchoirs', 'Liquide vaisselle', 'Tablettes lave-vaisselle',
  'Lessive', 'Adoucissant', 'Éponges', 'Sacs poubelle', 'Nettoyant multi-usage', 'Javel',
  'Savon', 'Gel douche', 'Shampoing', 'Après-shampoing', 'Dentifrice', 'Brosse à dents',
  'Déodorant', 'Rasoirs', 'Mousse à raser', 'Coton', 'Cotons-tiges', 'Crème hydratante',
  'Protections hygiéniques', 'Papier aluminium', 'Film alimentaire', 'Papier cuisson',
  // --- Bébé ---
  'Couches', 'Lingettes bébé', 'Lait infantile', 'Petits pots', 'Céréales bébé',
  // --- Animaux ---
  'Croquettes chien', 'Croquettes chat', 'Pâtée chat', 'Litière chat'
];
function populateShoppingSuggestions() {
  // Conservé pour compatibilité, mais les suggestions passent désormais par renderShoppingSuggestions (vrai menu déroulant, pas le datalist natif du navigateur)
}

function renderShoppingSuggestions(query) {
  const dropdown = document.getElementById('shoppingSuggestDropdown');
  if (!dropdown) return;
  const q = normalizeSearch(query).trim();
  if (!q) { dropdown.classList.add('hidden'); dropdown.innerHTML = ''; return; }

  const matches = PRODUCT_DB.filter(p => normalizeSearch(p).includes(q)).slice(0, 8);
  if (!matches.length) { dropdown.classList.add('hidden'); dropdown.innerHTML = ''; return; }

  dropdown.innerHTML = matches.map(p => {
    const meta = getArticleMeta(p);
    return `<div class="shop-suggest-item" onclick="selectShoppingSuggestion('${p.replace(/'/g,"\\'")}')">
      <span class="shop-suggest-emoji">${meta.emoji}</span>
...
function selectShoppingSuggestion(name) {
  const input = document.getElementById('shoppingQuickInput');
  input.value = name;
  document.getElementById('shoppingSuggestDropdown').classList.add('hidden');
  input.focus();
}
...
document.addEventListener('click', e => {
  ...
  const inlineDrop = document.getElementById('inlineProductSuggest');
  if (inlineDrop && !inlineDrop.contains(e.target) && e.target.tagName !== 'INPUT') inlineDrop.classList.add('hidden');
});
...
function renderArticlesEditor(articles) {
  ...
  container.innerHTML = (articles || []).map((a,i) => `
    <div class="checklist-item" style="position:relative">
      ...
      <input type="text" value="${escapeHtml(a.name||'')}" placeholder="Article" style="flex:1" autocomplete="off"
             oninput="renderInlineProductSuggestions(this, ${i})"
             onchange="updateArticle(${i},'name',this.value)">
...
// Menu déroulant de suggestions attaché à n'importe quel champ Article du formulaire
function renderInlineProductSuggestions(input, index) {
  let dropdown = document.getElementById('inlineProductSuggest');
  if (!dropdown) {
    dropdown = document.createElement('div');
    dropdown.id = 'inlineProductSuggest';
    dropdown.className = 'shop-suggest-dropdown inline-suggest';
    document.body.appendChild(dropdown);
  }
  const q = normalizeSearch(input.value).trim();
  if (!q) { dropdown.classList.add('hidden'); return; }

  const matches = PRODUCT_DB.filter(p => normalizeSearch(p).includes(q)).slice(0, 6);
  if (!matches.length) { dropdown.classList.add('hidden'); return; }

  dropdown.innerHTML = matches.map(p => {
    const meta = getArticleMeta(p);
    return `<div class="shop-suggest-item" onmousedown="pickInlineProduct(${index}, '${p.replace(/'/g,"\\'")}')" ontouchstart="pickInlineProduct(${index}, '${p.replace(/'/g,"\\'")}')">
      <span class="shop-suggest-emoji">${meta.emoji}</span><span>${escapeHtml(p)}</span>
    </div>`;
  }).join('');
...
function pickInlineProduct(index, name) {
  currentArticles[index].name = name;
  const memPrice = recallPrice(name);
  if (memPrice && !currentArticles[index].price) currentArticles[index].price = memPrice;
  document.getElementById('inlineProductSuggest')?.classList.add('hidden');
  renderArticlesEditor(currentArticles);
}
...
function parseQuickInput(text) {
  ...
  let cat = null;
  if (/\b(appeler|appel|téléphoner|tél)\b/.test(lower)) cat = 'appel';
  else if (/\b(rdv|rendez-vous|consultation|médecin|dentiste)\b/.test(lower)) cat = 'rdv';
  else if (/\b(anniversaire|anniv)\b/.test(lower)) cat = 'anniversaire';
  else if (/\b(course|courses|acheter|supermarché)\b/.test(lower)) cat = 'courses';
  else if (/\b(sport|muscu|yoga|footing|running)\b/.test(lower)) cat = 'sport';
  else if (/\b(facture|payer|échéance)\b/.test(lower)) cat = 'facture';

  let date = null;
  if (/\baujourd'?hui\b/.test(lower)) date = new Date(today);
  else if (/\bdemain\b/.test(lower)) { date = new Date(today); date.setDate(date.getDate()+1); }
  else if (/\baprès-demain\b/.test(lower)) { date = new Date(today); date.setDate(date.getDate()+2); }
  else {
    ...
    if (dayMatch) {
      ...
      date = new Date(today);
      do { date.setDate(date.getDate()+1); } while (date.getDay() !== targetDay);
    }
  }

  let time = null;
  const timeMatch = lower.match(/\b(\d{1,2})\s*h\s*(\d{0,2})\b/);
  if (timeMatch) time = `${timeMatch[1].padStart(2,'0')}:${(timeMatch[2]||'00').padStart(2,'0')}`;

  let recurrence = null;
  if (/\b(chaque|tous les|toutes les)\b/.test(lower)) {
    if (/\bjour\b/.test(lower)) recurrence = 'daily';
    else if (/\bsemaine\b/.test(lower)) recurrence = 'weekly';
    else if (/\bmois\b/.test(lower)) recurrence = 'monthly';
  }

  let title = text
    .replace(/\b(aujourd'?hui|demain|après-demain|chaque|tous les|toutes les)\b/gi, '')
  ...
  return {
    isEvent: !!(date && time),
    title, date, time, cat, recurrence
  };
}
...
async function quickAddTask(e) {
  ...
  if (parsed.isEvent) {
    await sbInsert('events', {
      ...
      time: parsed.time,
      cat: parsed.cat || 'autre',
      rec: parsed.recurrence || 'none'
    });
    toast(`Événement créé pour le ${parsed.date.toLocaleDateString('fr-FR',{day:'numeric',month:'short'})} à ${parsed.time} ✓`, 'success');
  } else {
    await sbInsert('tasks', {
      user_id: state.user.id,
      title: parsed.title,
      prio: 'normal',
      template: parsed.cat === 'appel' ? 'appel' : (parsed.cat === 'courses' ? 'courses' : (parsed.cat === 'sport' ? 'sport' : 'standard')),
      due: parsed.date ? parsed.date.toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
      recurrence_rule: parsed.recurrence,
      notes: ''
    });
    toast(parsed.date ? `Tâche ajoutée pour le ${parsed.date.toLocaleDateString('fr-FR',{day:'numeric',month:'short'})} ✓` : "Ajoutée pour aujourd'hui ✓", 'success');
  }
  input.value = '';
...
async function saveTask(e) {
  ...
  const obj = {
    ...
    objet: (() => {
      const sel = document.getElementById('taskSpecialite');
      if (!sel) return document.getElementById('taskObjet')?.value || null;
      if (sel.value === 'autre') return document.getElementById('taskSpecialiteAutreText')?.value || null;
      return sel.value ? sel.selectedOptions[0].textContent : null;
    })(),
...
function editTask(id) {
  ...
  document.getElementById('taskTime').value = t.due_time || '';
  ...
  if (document.getElementById('taskSpecialite') && t.objet) {
    const sel = document.getElementById('taskSpecialite');
    const match = [...sel.options].find(o => o.textContent === t.objet);
    if (match) { sel.value = match.value; }
    else { sel.value = 'autre'; document.getElementById('taskSpecialiteAutre').style.display = 'block'; document.getElementById('taskSpecialiteAutreText').value = t.objet; }
  }
...
function hapticTick() { if (navigator.vibrate) navigator.vibrate(10); }

async function toggleTask(id) {
  const t = state.tasks.find(x=>x.id===id);
  if (!t) return;
  hapticTick();
  const previousState = t.done;
  t.done = !t.done; // mise à jour optimiste immédiate, sensation instantanée
  renderCurrentView();
  const { error } = await sbUpdate('tasks', id, { done: t.done });
  if (error && sbAvailable) {
    // Le serveur a refusé la modification (contrairement à une simple coupure réseau, déjà couverte par le repli local) : on revient en arrière pour ne jamais afficher un état mensonger.
    t.done = previousState;
    renderCurrentView();
  }
}

function deleteTask(id) {
  deleteWithUndo('tasks', id, 'Tâche');
}

// ==================== RECETTES ====================
// ==================== BIBLIOTHÈQUE DE RECETTES INTÉGRÉES ====================
// Recettes classiques, décrites dans nos propres mots (aucun contenu tiers copié).
// base = nombre de personnes pour lequel les quantités sont données.
const RECIPE_LIBRARY = [
  { name:'Pâtes à la bolognaise', regime:['classique'], style:['traditionnel','rapide'], base:4, time:30,
    ingredients:[{name:'Pâtes',qty:400,unit:'g'},{name:'Bœuf haché',qty:400,unit:'g'},{name:'Sauce tomate',qty:500,unit:'g'},{name:'Oignons',qty:1,unit:''},{name:'Ail',qty:2,unit:'gousses'},{name:"Huile d'olive",qty:2,unit:'c.à.s'}] },
  { name:'Omelette aux herbes', regime:['classique','vegetarien','sans-gluten'], style:['rapide'], base:2, time:10,
    ingredients:[{name:'Œufs',qty:6,unit:''},{name:'Persil',qty:1,unit:'bouquet'},{name:'Beurre',qty:20,unit:'g'}] },
  { name:'Salade composée', regime:['classique','vegetarien','sans-gluten'], style:['rapide'], base:2, time:15,
    ingredients:[{name:'Salade',qty:1,unit:''},{name:'Tomates',qty:3,unit:''},{name:'Concombre',qty:1,unit:''},{name:'Feta',qty:100,unit:'g'},{name:'Olives',qty:50,unit:'g'},{name:"Huile d'olive",qty:3,unit:'c.à.s'}] },
  { name:'Soupe de légumes', regime:['classique','vegetarien','vegan','sans-gluten'], style:['batch','traditionnel'], base:6, time:40,
    ingredients:[{name:'Carottes',qty:4,unit:''},{name:'Poireaux',qty:2,unit:''},{name:'Pommes de terre',qty:3,unit:''},{name:'Céleri',qty:1,unit:''},{name:'Bouillon cube',qty:2,unit:''}] },
  { name:'Riz cantonais', regime:['classique'], style:['asiatique','rapide'],  base:4, time:25,
    ingredients:[{name:'Riz',qty:300,unit:'g'},{name:'Œufs',qty:2,unit:''},{name:'Petits pois',qty:150,unit:'g'},{name:'Jambon blanc',qty:150,unit:'g'},{name:'Sauce soja',qty:3,unit:'c.à.s'}] },
  { name:'Poulet rôti et pommes de terre', regime:['classique','sans-gluten'], style:['traditionnel','batch'], base:4, time:75,
    ingredients:[{name:'Poulet',qty:1,unit:''},{name:'Pommes de terre',qty:800,unit:'g'},{name:'Ail',qty:4,unit:'gousses'},{name:'Thym',qty:1,unit:'branche'},{name:"Huile d'olive",qty:3,unit:'c.à.s'}] },
  { name:'Curry de pois chiches', regime:['vegetarien','vegan','sans-gluten'], style:['batch','asiatique'], base:4, time:35,
    ingredients:[{name:'Pois chiches',qty:500,unit:'g'},{name:'Lait de coco',qty:400,unit:'ml'},{name:'Curry',qty:2,unit:'c.à.c'},{name:'Oignons',qty:1,unit:''},{name:'Tomates',qty:3,unit:''},{name:'Riz',qty:250,unit:'g'}] },
  { name:'Ratatouille', regime:['classique','vegetarien','vegan','sans-gluten'], style:['traditionnel','batch'], base:6, time:50,
    ingredients:[{name:'Aubergines',qty:2,unit:''},{name:'Courgettes',qty:3,unit:''},{name:'Poivrons',qty:2,unit:''},{name:'Tomates',qty:5,unit:''},{name:'Oignons',qty:2,unit:''},{name:"Huile d'olive",qty:4,unit:'c.à.s'}] },
  { name:'Saumon au four et légumes', regime:['classique','sans-gluten'], style:['rapide'], base:2, time:30,
    ingredients:[{name:'Saumon',qty:2,unit:'pavés'},{name:'Courgettes',qty:2,unit:''},{name:'Citrons',qty:1,unit:''},{name:"Huile d'olive",qty:2,unit:'c.à.s'}] },
  { name:'Chili sin carne', regime:['vegetarien','vegan','sans-gluten'], style:['batch','mexicain'], base:6, time:45,
    ingredients:[{name:'Haricots rouges',qty:500,unit:'g'},{name:'Maïs',qty:200,unit:'g'},{name:'Tomates',qty:400,unit:'g'},{name:'Poivrons',qty:2,unit:''},{name:'Oignons',qty:2,unit:''},{name:'Cumin',qty:1,unit:'c.à.c'},{name:'Riz',qty:300,unit:'g'}] },
  { name:'Gratin de courgettes', regime:['classique','vegetarien'], style:['traditionnel','batch'], base:4, time:50,
    ingredients:[{name:'Courgettes',qty:800,unit:'g'},{name:'Crème fraîche',qty:200,unit:'ml'},{name:'Fromage râpé',qty:150,unit:'g'},{name:'Œufs',qty:2,unit:''}] },
  { name:'Tajine de légumes', regime:['vegetarien','vegan'], style:['oriental','batch'], base:6, time:60,
    ingredients:[{name:'Carottes',qty:4,unit:''},{name:'Courgettes',qty:2,unit:''},{name:'Pois chiches',qty:400,unit:'g'},{name:'Semoule',qty:400,unit:'g'},{name:'Curcuma',qty:1,unit:'c.à.c'},{name:'Cumin',qty:1,unit:'c.à.c'},{name:'Oignons',qty:2,unit:''}] },
  { name:'Pâtes au pesto', regime:['classique','vegetarien'], style:['italien','rapide'], base:4, time:15,
    ingredients:[{name:'Pâtes',qty:400,unit:'g'},{name:'Basilic',qty:1,unit:'bouquet'},{name:'Parmesan',qty:80,unit:'g'},{name:'Pignons',qty:50,unit:'g'},{name:"Huile d'olive",qty:6,unit:'c.à.s'},{name:'Ail',qty:1,unit:'gousse'}] },
  { name:'Quiche lorraine', regime:['classique'], style:['traditionnel'], base:6, time:50,
    ingredients:[{name:'Lardons',qty:200,unit:'g'},{name:'Œufs',qty:4,unit:''},{name:'Crème fraîche',qty:250,unit:'ml'},{name:'Fromage râpé',qty:100,unit:'g'},{name:'Farine',qty:250,unit:'g'},{name:'Beurre',qty:125,unit:'g'}] },
  { name:'Buddha bowl quinoa', regime:['vegetarien','vegan','sans-gluten'], style:['rapide','batch'], base:2, time:25,
    ingredients:[{name:'Quinoa',qty:150,unit:'g'},{name:'Avocat',qty:1,unit:''},{name:'Pois chiches',qty:200,unit:'g'},{name:'Carottes',qty:2,unit:''},{name:'Épinards',qty:100,unit:'g'},{name:'Citrons',qty:1,unit:''}] },
  { name:'Wraps au poulet', regime:['classique'], style:['rapide','mexicain'], base:4, time:20,
    ingredients:[{name:'Wraps',qty:4,unit:''},{name:'Filets de poulet',qty:400,unit:'g'},{name:'Salade',qty:1,unit:''},{name:'Tomates',qty:2,unit:''},{name:'Mayonnaise',qty:3,unit:'c.à.s'}] },
  { name:'Lasagnes végétariennes', regime:['vegetarien'], style:['italien','batch'], base:6, time:70,
    ingredients:[{name:'Lasagnes',qty:250,unit:'g'},{name:'Courgettes',qty:3,unit:''},{name:'Aubergines',qty:2,unit:''},{name:'Sauce tomate',qty:600,unit:'g'},{name:'Mozzarella',qty:250,unit:'g'},{name:'Crème liquide',qty:200,unit:'ml'}] },
  { name:'Dahl de lentilles corail', regime:['vegetarien','vegan','sans-gluten'], style:['batch','asiatique'], base:4, time:35,
    ingredients:[{name:'Lentilles',qty:300,unit:'g'},{name:'Lait de coco',qty:400,unit:'ml'},{name:'Curcuma',qty:1,unit:'c.à.c'},{name:'Gingembre',qty:1,unit:'morceau'},{name:'Oignons',qty:1,unit:''},{name:'Riz',qty:250,unit:'g'}] },
  { name:'Steak frites salade', regime:['classique','sans-gluten'], style:['rapide','traditionnel'], base:2, time:20,
    ingredients:[{name:'Steak',qty:2,unit:''},{name:'Frites surgelées',qty:400,unit:'g'},{name:'Salade',qty:1,unit:''}] },
  { name:'Pâtes à l\'arrabbiata', regime:['classique','vegetarien','vegan'], style:['italien','rapide'], base:4, time:20,
    ingredients:[{name:'Pâtes',qty:400,unit:'g'},{name:'Sauce tomate',qty:500,unit:'g'},{name:'Ail',qty:3,unit:'gousses'},{name:'Huile d\'olive',qty:3,unit:'c.à.s'},{name:'Persil',qty:1,unit:'bouquet'}] },
  { name:'Couscous poulet-merguez', regime:['classique'], style:['oriental','traditionnel'], base:6, time:70,
    ingredients:[{name:'Semoule',qty:500,unit:'g'},{name:'Cuisses de poulet',qty:6,unit:''},{name:'Merguez',qty:6,unit:''},{name:'Carottes',qty:4,unit:''},{name:'Courgettes',qty:3,unit:''},{name:'Pois chiches',qty:400,unit:'g'},{name:'Navets',qty:2,unit:''}] }
];

const RECIPE_REGIMES = [
...
const RECIPE_STYLES = [
  ...
  { id:'rapide', label:'Rapide (-30 min)', emoji:'⚡' },
...
// ==================== MOTEUR D'IDÉES DE REPAS ====================
let ideaFilters = { regime: [], style: [] };
let assignContext = null; // { recipe, servings }

function openRecipeIdeas() {
  renderIdeaChips();
  renderIdeaResults();
  openModal('recipeIdeasModal');
}

function resetRecipeIdeaFilters() {
  ideaFilters = { regime: [], style: [] };
  renderIdeaChips();
  renderIdeaResults();
}

function toggleIdeaFilter(kind, id) {
  const arr = ideaFilters[kind];
...
function renderIdeaChips() {
  document.getElementById('ideaRegimeChips').innerHTML = RECIPE_REGIMES.map(r =>
    `<button type="button" class="idea-chip ${ideaFilters.regime.includes(r.id)?'active':''}" onclick="toggleIdeaFilter('regime','${r.id}')">${r.emoji} ${r.label}</button>`
  ).join('');
  document.getElementById('ideaStyleChips').innerHTML = RECIPE_STYLES.map(s =>
    `<button type="button" class="idea-chip ${ideaFilters.style.includes(s.id)?'active':''}" onclick="toggleIdeaFilter('style','${s.id}')">${s.emoji} ${s.label}</button>`
  ).join('');
}

function filteredRecipeIdeas() {
  return RECIPE_LIBRARY.filter(r => {
    const okRegime = !ideaFilters.regime.length || ideaFilters.regime.some(f => r.regime.includes(f));
    const okStyle = !ideaFilters.style.length || ideaFilters.style.some(f => r.style.includes(f));
    return okRegime && okStyle;
  });
}

function renderIdeaResults() {
  const list = filteredRecipeIdeas();
  document.getElementById('ideaCount').textContent = `${list.length} idée${list.length>1?'s':''}`;
  document.getElementById('ideaResults').innerHTML = list.map((r, i) => {
    const styleLabels = r.style.map(s => RECIPE_STYLES.find(x=>x.id===s)?.emoji || '').join(' ');
    return `
      <div class="idea-card" onclick="openAssignRecipe(${RECIPE_LIBRARY.indexOf(r)})">
        <div class="idea-card-main">
          <div class="idea-card-title">${escapeHtml(r.name)}</div>
          <div class="idea-card-meta">${styleLabels} · ⏱️ ${r.time} min · ${r.ingredients.length} ingrédients</div>
        </div>
  ...
  }).join('') || `<div class="empty-state" style="padding:24px">Aucune idée ne correspond à ces filtres — essaie d'en retirer un</div>`;
}

function openAssignRecipe(libIndex) {
  const recipe = RECIPE_LIBRARY[libIndex];
  assignContext = { recipe, servings: recipe.base };
  document.getElementById('assignRecipeTitle').textContent = recipe.name;
  document.getElementById('assignServings').textContent = recipe.base;

  // Jours de la semaine affichée
  const monday = getMondayOf(state.menuWeekDate);
  const daySelect = document.getElementById('assignDay');
  daySelect.innerHTML = [...Array(7)].map((_,i) => {
    const d = new Date(monday); d.setDate(d.getDate()+i);
    const iso = d.toISOString().split('T')[0];
    return `<option value="${iso}">${d.toLocaleDateString('fr-FR',{weekday:'long',day:'numeric',month:'short'})}</option>`;
  }).join('');

  renderAssignPreview();
  closeModal('recipeIdeasModal');
  openModal('recipeAssignModal');
}

function changeAssignServings(delta) {
  if (!assignContext) return;
  assignContext.servings = Math.max(1, Math.min(20, assignContext.servings + delta));
  document.getElementById('assignServings').textContent = assignContext.servings;
  renderAssignPreview();
}
...
function renderAssignPreview() {
  if (!assignContext) return;
  const { recipe, servings } = assignContext;
  const scaled = scaledIngredients(recipe, servings);
  document.getElementById('assignPreview').innerHTML =
    `<strong style="color:var(--text-secondary)">Ingrédients pour ${servings} personne${servings>1?'s':''} :</strong><br>` +
    scaled.map(i => `• ${escapeHtml(i.name)} — ${i.qty}${i.unit ? ' '+i.unit : ''}`).join('<br>');
}

async function confirmAssignRecipe() {
  if (!assignContext) return;
  const { recipe, servings } = assignContext;
  const dateStr = document.getElementById('assignDay').value;
  const slot = document.getElementById('assignSlot').value;

  // 1. Créer la recette dans la bibliothèque personnelle, avec les quantités ajustées
  const { data } = await sbInsert('recipes', {
    user_id: state.user.id,
    name: `${recipe.name} (${servings} pers.)`,
    ingredients: scaledIngredients(recipe, servings)
  });
  const newRecipe = data?.[0];
  if (!newRecipe) { toast('Impossible d\'enregistrer la recette', 'error'); return; }

  // 2. L'assigner au créneau choisi (remplace s'il y a déjà quelque chose)
  const existing = state.menus.find(m => m.date === dateStr && m.slot === slot);
  const obj = { user_id: state.user.id, date: dateStr, slot, recipe_id: newRecipe.id, free_text: null };
  if (existing) await sbUpdate('menus', existing.id, obj); else await sbInsert('menus', obj);

  closeModal('recipeAssignModal');
  toast(`${recipe.name} ajouté à ta semaine ✓`, 'success');
  assignContext = null;
  await loadAllData();
  renderCurrentView();
}
...
function renderRecipeIngredientsEditor() {
  document.getElementById('recipeIngredientsList').innerHTML = currentRecipeIngredients.map((ing,i) => `
    <div class="checklist-item">
      <input type="text" value="${escapeHtml(ing.name||'')}" placeholder="Ingrédient" style="flex:2" autocomplete="off"
             oninput="renderInlineIngredientSuggestions(this, ${i})"
             onchange="updateRecipeIngredient(${i},'name',this.value)">
...
function renderInlineIngredientSuggestions(input, index) {
  let dropdown = document.getElementById('inlineProductSuggest');
  if (!dropdown) {
    dropdown = document.createElement('div');
    dropdown.id = 'inlineProductSuggest';
  ...
  dropdown.innerHTML = matches.map(p => {
    const meta = getArticleMeta(p);
    return `<div class="shop-suggest-item" onmousedown="pickInlineIngredient(${index}, '${p.replace(/'/g,"\\'")}')" ontouchstart="pickInlineIngredient(${index}, '${p.replace(/'/g,"\\'")}')">
      <span class="shop-suggest-emoji">${meta.emoji}</span><span>${escapeHtml(p)}</span>
    </div>`;
  }).join('');
...
function pickInlineIngredient(index, name) {
  currentRecipeIngredients[index].name = name;
  document.getElementById('inlineProductSuggest')?.classList.add('hidden');
  renderRecipeIngredientsEditor();
}
...
function deleteRecipe(id) {
  deleteWithUndo('recipes', id, 'Recette');
}
...
// ==================== PROJETS ====================
const PROJECT_SUBTYPE_LABELS = { credit: '🏦 Crédit', immobilier: '🏠 Immobilier', vehicule: '🚗 Véhicule', formation: '🎓 Formation' };
...
// ==================== SOUS-MODULES DE PROJET ====================
let currentProjectChecklist = [];

const PROJECT_SUBTYPE_FIELDS = {
  credit: () => `
    <div class="grid-2">
      <div class="form-group"><label>Type de crédit</label><select id="projSub_typeCredit"><option value="conso">Conso</option><option value="immo">Immobilier</option><option value="auto">Auto</option><option value="travaux">Travaux</option></select></div>
      <div class="form-group"><label>Organisme prêteur</label><input type="text" id="projSub_organisme"></div>
    </div>
    <div class="form-group"><label>Statut du dossier</label>
      <select id="projSub_statut"><option value="depose">Dossier déposé</option><option value="etude">En cours d'étude</option><option value="accepte">Accepté</option><option value="refuse">Refusé</option><option value="remboursement">En remboursement</option></select>
    </div>
    ${projectChecklistHTML(['Justificatifs de revenus', 'Relevés bancaires', "Pièce d'identité", 'Justificatif de domicile'])}
  `,
  immobilier: () => `
    <div class="grid-2">
      <div class="form-group"><label>Budget max (€)</label><input type="number" id="projSub_budget"></div>
      <div class="form-group"><label>Apport (€)</label><input type="number" id="projSub_apport"></div>
    </div>
    <div class="form-group"><label>Statut</label>
      <select id="projSub_statut"><option value="recherche">Recherche</option><option value="visite">Visites en cours</option><option value="offre">Offre faite</option><option value="compromis">Compromis signé</option><option value="pret">Prêt en cours</option><option value="acte">Acte signé</option></select>
    </div>
    ${projectChecklistHTML(['Diagnostic technique', 'Assurance emprunteur', 'Rendez-vous notaire', 'Déblocage des fonds'])}
  `,
  vehicule: () => `
    <div class="grid-2">
      <div class="form-group"><label>Budget (€)</label><input type="number" id="projSub_budget"></div>
      <div class="form-group"><label>Neuf ou occasion</label><select id="projSub_etat"><option value="neuf">Neuf</option><option value="occasion">Occasion</option></select></div>
    </div>
    <div class="form-group"><label>Financement</label><select id="projSub_financement"><option value="comptant">Comptant</option><option value="credit">Crédit</option><option value="leasing">Leasing / LOA</option></select></div>
    ${projectChecklistHTML(['Essai routier', 'Contrôle technique vérifié', 'Assurance souscrite', 'Carte grise'])}
  `,
  formation: () => `
    <div class="grid-2">
      <div class="form-group"><label>Organisme</label><input type="text" id="projSub_organisme"></div>
      <div class="form-group"><label>Coût (€)</label><input type="number" id="projSub_budget"></div>
    </div>
    <div class="form-group"><label>Financement</label><select id="projSub_financement"><option value="cpf">CPF</option><option value="employeur">Employeur</option><option value="autofinance">Autofinancé</option></select></div>
    ${projectChecklistHTML(['Candidature envoyée', 'Inscription validée', 'Examen final', 'Certification obtenue'])}
  `
};

function projectChecklistHTML(defaultItems) {
  if (!currentProjectChecklist.length) currentProjectChecklist = defaultItems.map(label => ({ label, done: false }));
  return `
    <div class="form-group">
      <label>Suivi</label>
      <div id="projChecklistList">${renderProjectChecklistItems()}</div>
      <button type="button" class="btn btn-sm mt-4" onclick="addProjectChecklistItem()">+ Étape</button>
    </div>`;
}
function renderProjectChecklistItems() {
  return currentProjectChecklist.map((item, i) => `
    <div class="checklist-item">
      <input type="checkbox" ${item.done?'checked':''} onchange="currentProjectChecklist[${i}].done=this.checked">
      <input type="text" value="${escapeHtml(item.label)}" style="flex:1" onchange="currentProjectChecklist[${i}].label=this.value">
      <button type="button" class="btn btn-sm btn-ghost" onclick="removeProjectChecklistItem(${i})">×</button>
    </div>`).join('');
}
function addProjectChecklistItem() { currentProjectChecklist.push({label:'', done:false}); document.getElementById('projChecklistList').innerHTML = renderProjectChecklistItems(); }
function removeProjectChecklistItem(i) { currentProjectChecklist.splice(i,1); document.getElementById('projChecklistList').innerHTML = renderProjectChecklistItems(); }

function updateProjectSubtype(preserveChecklist) {
  const subtype = document.getElementById('projSubtype').value;
  if (!preserveChecklist) currentProjectChecklist = [];
  document.getElementById('projSubtypeFields').innerHTML = subtype && PROJECT_SUBTYPE_FIELDS[subtype] ? PROJECT_SUBTYPE_FIELDS[subtype]() : '';
}

function collectProjectSubtypeData(subtype) {
  if (!subtype) return null;
  const data = { checklist: currentProjectChecklist };
  document.querySelectorAll('#projSubtypeFields [id^="projSub_"]').forEach(el => {
    data[el.id.replace('projSub_', '')] = el.value;
  });
  return data;
}

const PROJECT_CAT_META = {
  vie: { label: 'Vie', color: 'var(--perso)', icon: '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="8" r="4"/><path d="M4 21v-1a8 8 0 0 1 16 0v1"/></svg>' },
  argent: { label: 'Argent', color: 'var(--invest)', icon: '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>' }
};
    ...
    <div class="project-card animate-fade" style="border-left:3px solid ${meta.color}">
      <div class="project-header">
        <div>
          ...
          <span class="badge badge-${p.cat}">${p.cat==='vie'?'Vie':'Argent'}</span>
          ${p.subtype ? `<span class="badge" style="background:var(--s2);color:var(--text-secondary)">${PROJECT_SUBTYPE_LABELS[p.subtype]||p.subtype}</span>` : ''}
        </div>
        <span class="project-phase">${p.phase}</span>
      </div>
      <div class="project-next">→ ${escapeHtml(p.next || 'Aucune action définie')}</div>
      ${p.subtype_data && p.subtype_data.checklist && p.subtype_data.checklist.length ? `
        <div style="font-size:0.78rem;color:var(--text-muted);margin-top:6px">✅ ${p.subtype_data.checklist.filter(c=>c.done).length}/${p.subtype_data.checklist.length} étapes</div>
      ` : ''}
      ${p.budget_target > 0 ? `
...
async function saveProject(e) {
  ...
  const obj = {
    ...
    subtype_data: collectProjectSubtypeData(subtype),
...
function editProject(id) {
  ...
  document.getElementById('projSubtype').value = p.subtype || '';
  currentProjectChecklist = (p.subtype_data && p.subtype_data.checklist) ? JSON.parse(JSON.stringify(p.subtype_data.checklist)) : [];
  updateProjectSubtype(true);
  if (p.subtype_data) {
    Object.keys(p.subtype_data).forEach(k => {
      if (k === 'checklist') return;
      const el = document.getElementById('projSub_' + k);
      if (el) el.value = p.subtype_data[k];
    });
  }
...
function deleteProject(id) {
  deleteWithUndo('projects', id, 'Projet');
}

// ==================== WALLET ====================
const ASSET_TYPE_LABELS = {
  liquidites: { label: 'Liquidités', icon: '💶', color: '#7A9EC4' },
  bourse: { label: 'Bourse / ETF', icon: '📈', color: '#D9A441' },
  crypto: { label: 'Crypto', icon: '🪙', color: '#E0B458' },
  immobilier: { label: 'Immobilier locatif', icon: '🏠', color: '#7CB69A' },
  credit: { label: 'Crédit', icon: '🏦', color: '#D17A70' },
  physique: { label: 'Projet physique', icon: '🛠️', color: '#A186D0' },
  revenu: { label: 'Revenus', icon: '💰', color: '#7CB69A' },
  charge: { label: 'Charges récurrentes', icon: '🧾', color: '#E0B458' },
  autre: { label: 'Autre', icon: '📦', color: '#8D99A8' }
};
// Types qui relèvent du budget mensuel plutôt que du patrimoine
const BUDGET_TYPES = ['revenu', 'charge', 'credit'];

// Un instantané par jour maximum : sans historique, aucun graphique d'évolution n'est possible plus tard.
let _snapshotCheckedToday = false;
async function maybeSnapshotWallet() {
  if (_snapshotCheckedToday || !state.user) return;
  _snapshotCheckedToday = true;
  const today = new Date().toISOString().split('T')[0];
  const existing = await sbSelect('wallet_snapshots', {});
  if (existing.some(s => s.snapshot_date === today)) return; // déjà fait aujourd'hui
  ...
  const details = {};
  state.wallet.forEach(a => { const t = a.type||'autre'; details[t] = (details[t]||0) + assetLineValue(a); });

  await sbInsert('wallet_snapshots', {
    user_id: state.user.id,
    snapshot_date: today,
...
function animateWalletNumber(target) {
  const el = document.getElementById('walletHeroNumber');
  if (!el) return;
...
// Répartition détaillée : où part l'argent chaque mois
function renderChargeBreakdown(charges, debts, totalRevenus) {
  const items = [
    ...charges.map(c => ({ name: c.name, amount: parseFloat(c.value)||0, color: 'var(--warn)' })),
    ...debts.filter(d => (d.status||'actif')==='actif' && d.monthly > 0)
            .map(d => ({ name: d.name, amount: parseFloat(d.monthly)||0, color: 'var(--urgent)' }))
  ...
  return `
    <div class="charge-breakdown">
      <div class="charge-breakdown-title">Où part ton argent</div>
      <div class="charge-stack">
        ${items.map(i => `<div class="charge-stack-seg" style="width:${(i.amount/total*100)}%;background:${i.color}" title="${escapeHtml(i.name)}"></div>`).join('')}
      </div>
      ${items.map(i => {
        const pctTotal = total > 0 ? (i.amount/total*100) : 0;
        const pctRev = totalRevenus > 0 ? (i.amount/totalRevenus*100) : null;
        return `<div class="charge-row">
          <span class="charge-dot" style="background:${i.color}"></span>
          <span class="charge-name">${escapeHtml(i.name)}</span>
          <span class="charge-amount mono">${fmtMoney(i.amount)}</span>
          <span class="charge-pct">${pctRev !== null ? pctRev.toFixed(0)+'% des revenus' : pctTotal.toFixed(0)+'%'}</span>
        </div>`;
      }).join('')}
    </div>`;
}
...
function renderWallet() {
  renderGoals();
  // Patrimoine : on exclut tout ce qui relève du budget mensuel (revenus, charges, crédits)
  const assets = state.wallet.filter(a => !BUDGET_TYPES.includes(a.type));
  const debts = state.wallet.filter(a => a.type === 'credit');
  const revenus = state.wallet.filter(a => a.type === 'revenu' && (a.status||'actif') === 'actif');
  const charges = state.wallet.filter(a => a.type === 'charge' && (a.status||'actif') === 'actif');
  ...
  const totalMonthlyDebt = debts.filter(a=>(a.status||'actif')==='actif').reduce((s,a) => s + (parseFloat(a.monthly) || 0), 0);
  const totalMonthlyRent = state.wallet.filter(a=>a.type==='immobilier').reduce((s,a) => s + (parseFloat(a.rent)||0), 0);

  // --- Budget mensuel façon appli bancaire ---
  const totalRevenus = revenus.reduce((s,a) => s + (parseFloat(a.value)||0), 0) + totalMonthlyRent;
  const totalCharges = charges.reduce((s,a) => s + (parseFloat(a.value)||0), 0) + totalMonthlyDebt;
  const resteAVivre = totalRevenus - totalCharges;
  const tauxEndettement = totalRevenus > 0 ? (totalMonthlyDebt / totalRevenus * 100) : null;
  const partCharges = totalRevenus > 0 ? Math.min(100, (totalCharges / totalRevenus * 100)) : 0;

  // --- Donut d'allocation par catégorie (CSS pur, sans librairie) ---
  const byTypeTotals = {};
  assets.forEach(a => { const t = a.type||'autre'; byTypeTotals[t] = (byTypeTotals[t]||0) + assetLineValue(a); });
  const grandTotal = Object.values(byTypeTotals).reduce((s,v)=>s+v,0);
  let cursor = 0;
  const segments = [];
  const legend = [];
  Object.keys(byTypeTotals).filter(t=>byTypeTotals[t]>0).sort((a,b)=>byTypeTotals[b]-byTypeTotals[a]).forEach(t => {
    const meta = ASSET_TYPE_LABELS[t] || {label:t, color:'#8D99A8'};
    const pct = (byTypeTotals[t] / grandTotal) * 100;
    segments.push(`${meta.color} ${cursor}% ${cursor+pct}%`);
    legend.push(`<div class="wallet-hero-legend-item"><span class="wallet-hero-legend-dot" style="background:${meta.color}"></span>${meta.label} · ${Math.round(pct)}%</div>`);
    cursor += pct;
  });
  const donutStyle = grandTotal > 0 ? `background: conic-gradient(${segments.join(', ')})` : '';

  document.getElementById('walletHero').innerHTML = `
    <div class="wallet-donut ${grandTotal>0?'':'wallet-donut-empty'}" style="${donutStyle}">
      <div class="wallet-donut-center">${grandTotal>0 ? `<div class="pct">${assets.length}</div><div style="font-size:0.65rem;color:var(--text-muted)">ligne${assets.length>1?'s':''}</div>` : `<div style="font-size:1.3rem">💤</div>`}</div>
    </div>
    <div class="wallet-hero-main">
      <div class="wallet-hero-label">Patrimoine net</div>
      <div class="wallet-hero-number" id="walletHeroNumber">0 €</div>
      <div class="wallet-hero-sub">
        <span>Brut <b>${fmtMoney(totalAssets)}</b></span>
        <span>Dettes <b>${fmtMoney(totalDebts)}</b></span>
        <span style="color:${pnl>=0?'var(--ok)':'var(--urgent)'}">${pnl>=0?'▲':'▼'} ${fmtMoney(Math.abs(pnl))}${globalReturnPct!==null ? ' ('+(globalReturnPct>=0?'+':'')+globalReturnPct.toFixed(1)+'%)' : ''}</span>
      </div>
      ${legend.length ? `<div class="wallet-hero-legend">${legend.join('')}</div>` : ''}
    </div>
  `;
  animateWalletNumber(netWorth);

  // --- Bloc budget mensuel (visible seulement si des revenus/charges sont saisis) ---
  const budgetBox = document.getElementById('walletBudget');
  if (totalRevenus > 0 || totalCharges > 0) {
    budgetBox.classList.remove('hidden');
    budgetBox.innerHTML = `
      <div class="budget-head">
        <div>
          <div class="budget-label">Reste à vivre ce mois</div>
          <div class="budget-number" style="color:${resteAVivre>=0?'var(--ok)':'var(--urgent)'}">${fmtMoney(resteAVivre)}</div>
        </div>
        ${tauxEndettement !== null ? `<div class="budget-badge ${tauxEndettement>35?'over':''}">Endettement ${tauxEndettement.toFixed(0)}%</div>` : ''}
      </div>
      <div class="budget-bar"><div class="budget-bar-fill" style="width:${partCharges}%;background:${partCharges>85?'var(--urgent)':(partCharges>60?'var(--warn)':'var(--ok)')}"></div></div>
      <div class="budget-legend">
        <span>💰 Revenus <b>${fmtMoney(totalRevenus)}</b></span>
        <span>🧾 Charges <b>${fmtMoney(totalCharges)}</b></span>
      </div>
      ${renderChargeBreakdown(charges, debts, totalRevenus)}
      ${tauxEndettement !== null && tauxEndettement > 35 ? `<div class="budget-alert">⚠️ Ton taux d'endettement dépasse 35 %, le seuil généralement retenu par les banques.</div>` : ''}
    `;
  } else {
  ...
  document.getElementById('walletSummary').innerHTML = `
    ${totalRevenus > 0 ? `<div class="wallet-card"><div class="wallet-value" style="color:var(--ok)">${fmtMoney(totalRevenus)}</div><div class="wallet-label">Revenus / mois</div></div>` : ''}
    ${totalCharges > 0 ? `<div class="wallet-card"><div class="wallet-value" style="color:var(--warn)">${fmtMoney(totalCharges)}</div><div class="wallet-label">Charges / mois</div></div>` : ''}
    <div class="wallet-card"><div class="wallet-value">${fmtMoney(totalMonthlyDebt)}</div><div class="wallet-label">Dont crédits / mois</div></div>
    <div class="wallet-card"><div class="wallet-value">${state.wallet.length}</div><div class="wallet-label">Ligne${state.wallet.length>1?'s':''} au total</div></div>
  `;
...
function goalCurrentAmount(g) {
  return (g.linked_asset_ids || []).reduce((s, id) => {
    const a = state.wallet.find(x => x.id === id);
    return s + (a ? assetLineValue(a) : 0);
  }, 0);
}

async function projectionMonthlyRate() {
  const snaps = await sbSelect('wallet_snapshots', { order: { column: 'snapshot_date', asc: true } });
  if (snaps.length < 2) return null;
  const first = snaps[0], last = snaps[snaps.length - 1];
  const days = (new Date(last.snapshot_date) - new Date(first.snapshot_date)) / 86400000;
  if (days < 1) return null;
  return (last.net_worth - first.net_worth) / days * 30; // évolution moyenne par mois
}

async function renderGoals() {
  if (!state.goals.length) {
    document.getElementById('goalsContainer').innerHTML = `<div class="empty-state" style="padding:20px"><p>Aucun objectif défini — relie un projet à ton patrimoine pour suivre ta progression réelle</p></div>`;
    return;
  }
  const monthlyRate = await projectionMonthlyRate();
  document.getElementById('goalsContainer').innerHTML = state.goals.map(g => {
    ...
    let projectionText = '';
    if (remaining <= 0) {
      projectionText = 'Objectif atteint 🎉';
      ...
      const projStr = projDate.toLocaleDateString('fr-FR', { month:'long', year:'numeric' });
      if (g.target_date) {
        const target = new Date(g.target_date);
        const lateMonths = Math.round((projDate - target) / (30*86400000));
        projectionText = lateMonths > 1 ? `À ce rythme : ${projStr}, soit ${lateMonths} mois de retard sur ton objectif` : (lateMonths < -1 ? `À ce rythme : ${projStr}, en avance sur ton objectif !` : `À ce rythme : ${projStr}, dans les temps`);
      } else {
        projectionText = `À ce rythme : atteint vers ${projStr}`;
      }
    } else {
      projectionText = "Pas encore assez d'historique pour une projection fiable (revient dans quelques jours)";
    }
    const project = state.projects.find(p => p.id === g.project_id);
    const ringColor = pct >= 100 ? 'var(--ok)' : 'var(--gold)';
    return `
      <div class="card animate-fade" style="margin-bottom:12px;display:flex;gap:18px;align-items:center">
        <div class="goal-ring" style="background: conic-gradient(${ringColor} ${pct}%, var(--s2) ${pct}%)">
          <div class="goal-ring-center">${pct}%</div>
        </div>
        <div style="flex:1;min-width:0">
          <div style="display:flex;justify-content:space-between;align-items:flex-start">
            <div>
              <div style="font-weight:600">${escapeHtml(g.title)}</div>
              ${project ? `<div style="font-size:0.78rem;color:var(--text-muted)">📁 ${escapeHtml(project.title)}</div>` : ''}
            </div>
            <button class="btn btn-sm btn-ghost" onclick="deleteGoalConfirm('${g.id}')">🗑️</button>
          </div>
          <div style="font-size:0.85rem;color:var(--text-secondary);margin-top:6px">${fmtMoney(current)} / ${fmtMoney(g.target_amount)}</div>
          <div style="font-size:0.8rem;color:var(--text-muted);margin-top:4px">${projectionText}</div>
        </div>
      </div>`;
  }).join('');
}
...
function openGoalModal() {
  document.getElementById('goalModal').querySelector('form').reset();
  const projSelect = document.getElementById('goalProject');
  projSelect.innerHTML = '<option value="">Aucun</option>' + state.projects.map(p => `<option value="${p.id}">${escapeHtml(p.title)}</option>`).join('');
  document.getElementById('goalAssetsList').innerHTML = state.wallet.filter(a=>a.type!=='credit').map(a => `
    <label style="display:flex;align-items:center;gap:8px;font-weight:400;text-transform:none">
      <input type="checkbox" value="${a.id}" style="width:auto"> ${escapeHtml(a.name)}
    </label>`).join('') || '<p style="font-size:0.85rem;color:var(--text-muted)">Aucun actif à relier pour l\'instant</p>';
  openModal('goalModal');
}

async function saveGoal(e) {
  e.preventDefault();
  const linkedIds = [...document.querySelectorAll('#goalAssetsList input:checked')].map(i => i.value);
  await sbInsert('goals', {
    user_id: state.user.id,
    ...
    project_id: document.getElementById('goalProject').value || null,
    linked_asset_ids: linkedIds
  });
...
function deleteGoalConfirm(id) {
  deleteWithUndo('goals', id, 'Objectif', () => renderGoals());
}
...
function renderWalletTable() {
  const rows = state.wallet.filter(a => (a.status||'actif') === 'actif').map(a => {
    ...
    const val = parseFloat(a.value_current || a.value || 0);
    const isBudget = BUDGET_TYPES.includes(a.type);
    const displayValue = isBudget ? val : (val * qty);
    const returnPct = (!isBudget && buy > 0) ? ((val-buy)/buy*100) : null;
    return { a, displayValue, returnPct, isBudget };
  }).sort((x,y) => y.displayValue - x.displayValue);

  const rowsHtml = rows.map(({a, displayValue, returnPct, isBudget}) => {
    const meta = ASSET_TYPE_LABELS[a.type] || {icon:'📦', label:a.type};
    const pctClass = returnPct === null ? 'pct-flat' : (returnPct >= 0 ? 'pct-up' : 'pct-down');
    // Pour un revenu ou une charge, on affiche la périodicité plutôt qu'une performance qui n'a pas de sens
    const lastCol = isBudget
      ? `<span class="pct-badge ${a.type==='revenu'?'pct-up':'pct-down'}">${a.type==='revenu'?'+':'−'} / mois</span>`
      : `<span class="pct-badge ${pctClass}">${returnPct === null ? '—' : (returnPct>=0?'▲ +':'▼ ')+Math.abs(returnPct).toFixed(1)+'%'}</span>`;
    return `
      <tr onclick="openAssetDetail('${a.id}')">
...
function renderWalletDetailList() {
  ...
  document.getElementById('walletDetailList').innerHTML = items.map(a => {
    ...
    return `
    <div class="task-item animate-fade" style="cursor:pointer" onclick="openAssetDetail('${a.id}')">
      <div class="task-content">
...
async function updateAssetFields() {
  ...
  const isBudget = BUDGET_TYPES.includes(t);

  document.getElementById('assetRentGroup').style.display = t === 'immobilier' ? 'block' : 'none';
  document.getElementById('assetCreditGroup').style.display = t === 'credit' ? 'grid' : 'none';
  // Pour un revenu ou une charge, la quantité et le prix d'achat n'ont aucun sens
  document.getElementById('assetQtyGroup').style.display = isBudget ? 'none' : 'block';
  document.getElementById('assetPriceBuyGroup').style.display = isBudget ? 'none' : 'block';

  const valueLabel = document.getElementById('assetValueLabel');
  const monthlyLabel = document.getElementById('assetMonthlyLabel');
  const monthlyGroup = document.getElementById('assetMonthlyGroup');

  if (t === 'revenu') {
    valueLabel.textContent = 'Montant mensuel (€)';
    monthlyGroup.style.display = 'none';
  } else if (t === 'charge') {
    valueLabel.textContent = 'Montant mensuel (€)';
    monthlyGroup.style.display = 'none';
  } else if (t === 'credit') {
    valueLabel.textContent = 'Capital restant dû (€)';
    monthlyLabel.textContent = 'Mensualité (€)';
    monthlyGroup.style.display = 'block';
  } else {
    valueLabel.textContent = 'Valeur actuelle (€)';
    monthlyLabel.textContent = 'Mensualité (crédit)';
    monthlyGroup.style.display = 'block';
  }
}
...
async function saveAsset(e) {
  ...
  const obj = {
    ...
    duration_months: parseInt(document.getElementById('assetDuration').value) || null,
    rate: parseFloat(document.getElementById('assetRate').value) || null,
  ...
  if (state.editingType === 'asset') {
    ...
    const { data } = await sbInsert('wallet_assets', obj);
    // Premier point d'historique, pour que l'évolution soit suivie dès la création
    const created = data?.[0];
    if (created && obj.value) {
      await sbInsert('asset_history', { asset_id: created.id, user_id: state.user.id, value: obj.value, note: 'Création' });
    }
...
async function openAssetDetail(id) {
  ...
  if (!a) return;
  detailAssetId = id;
  const meta = ASSET_TYPE_LABELS[a.type] || { label: a.type, icon: '📦' };
  document.getElementById('assetDetailTitle').textContent = `${meta.icon} ${a.name}`;

  const isBudget = BUDGET_TYPES.includes(a.type);
  const qty = parseFloat(a.quantity || 1);
  const buy = parseFloat(a.price_buy || 0);
  const val = parseFloat(a.value || 0);
  const totalVal = isBudget ? val : val * qty;
  const pnl = isBudget ? null : (val - buy) * qty;
  const returnPct = (!isBudget && buy > 0) ? ((val - buy) / buy * 100) : null;

  // Suivi crédit : reste à payer et date de fin estimée
  let creditInfo = '';
  if (a.type === 'credit' && a.monthly > 0) {
    const months = a.duration_months ? parseInt(a.duration_months) : Math.ceil(val / parseFloat(a.monthly));
    const endDate = new Date(); endDate.setMonth(endDate.getMonth() + months);
    const totalRemaining = parseFloat(a.monthly) * months;
    creditInfo = `
      <div class="detail-grid">
        <div class="detail-item"><div class="detail-item-label">Mensualité</div><div class="detail-item-value">${fmtMoney(a.monthly)}</div></div>
        <div class="detail-item"><div class="detail-item-label">Mois restants</div><div class="detail-item-value">${months}</div></div>
        <div class="detail-item"><div class="detail-item-label">Reste à payer</div><div class="detail-item-value">${fmtMoney(totalRemaining)}</div></div>
        <div class="detail-item"><div class="detail-item-label">Fin estimée</div><div class="detail-item-value">${endDate.toLocaleDateString('fr-FR',{month:'short',year:'numeric'})}</div></div>
        ${a.rate ? `<div class="detail-item"><div class="detail-item-label">Taux</div><div class="detail-item-value">${a.rate} %</div></div>` : ''}
      </div>`;
  }

  const history = await sbSelect('asset_history', { order: { column: 'recorded_at', asc: false } });
  const lineHistory = history.filter(h => h.asset_id === id).slice(0, 12);

  document.getElementById('assetDetailBody').innerHTML = `
    <div class="detail-hero">
      <div class="detail-hero-label">${isBudget ? (a.type==='revenu'?'Montant mensuel':'Montant mensuel') : 'Valeur actuelle'}</div>
      <div class="detail-hero-value">${fmtMoney(totalVal)}</div>
      ${returnPct !== null ? `<div style="color:${pnl>=0?'var(--ok)':'var(--urgent)'};font-size:0.9rem;margin-top:4px">${pnl>=0?'▲ +':'▼ '}${fmtMoney(Math.abs(pnl))} (${returnPct>=0?'+':''}${returnPct.toFixed(1)} %)</div>` : ''}
    </div>

    ${creditInfo}

    <div class="detail-grid">
      <div class="detail-item"><div class="detail-item-label">Type</div><div class="detail-item-value">${meta.label}</div></div>
      <div class="detail-item"><div class="detail-item-label">Statut</div><div class="detail-item-value">${(a.status||'actif')==='actif'?'Actif':'Terminé'}</div></div>
      ${a.platform ? `<div class="detail-item"><div class="detail-item-label">Plateforme</div><div class="detail-item-value">${escapeHtml(a.platform)}</div></div>` : ''}
      ${a.start_date ? `<div class="detail-item"><div class="detail-item-label">Depuis</div><div class="detail-item-value">${durationSince(a.start_date).replace('depuis ','')}</div></div>` : ''}
      ${!isBudget && buy>0 ? `<div class="detail-item"><div class="detail-item-label">Prix d'achat</div><div class="detail-item-value">${fmtMoney(buy)}</div></div>` : ''}
      ${!isBudget && qty!==1 ? `<div class="detail-item"><div class="detail-item-label">Quantité</div><div class="detail-item-value">${qty}</div></div>` : ''}
      ${a.rent>0 ? `<div class="detail-item"><div class="detail-item-label">Loyer / mois</div><div class="detail-item-value">${fmtMoney(a.rent)}</div></div>` : ''}
    </div>

    ${a.notes ? `<div class="detail-notes">${escapeHtml(a.notes)}</div>` : ''}

    <div class="detail-section-title">Historique des valeurs</div>
    ${lineHistory.length ? `
      ${renderAssetSparkline(lineHistory)}
      <div class="detail-history">
        ${lineHistory.map(h => `
          <div class="detail-history-row">
            <span>${new Date(h.recorded_at).toLocaleDateString('fr-FR',{day:'numeric',month:'short',year:'2-digit'})}</span>
            <span class="mono">${fmtMoney(h.value)}</span>
            ${h.note ? `<span style="color:var(--text-muted);font-size:0.78rem">${escapeHtml(h.note)}</span>` : '<span></span>'}
          </div>`).join('')}
      </div>` : `<div style="font-size:0.85rem;color:var(--text-muted);padding:12px 0">Aucun relevé pour l'instant — utilise "Actualiser la valeur" pour commencer à suivre l'évolution.</div>`}
  `;
  openModal('assetDetailModal');
}

// Petite courbe d'évolution en SVG, sans aucune librairie externe
function renderAssetSparkline(history) {
  const pts = [...history].reverse().map(h => parseFloat(h.value));
  ...
  const color = rising ? 'var(--ok)' : 'var(--urgent)';
  return `<svg viewBox="0 0 ${w} ${h}" preserveAspectRatio="none" style="width:100%;height:60px;margin:10px 0">
...
function editAssetFromDetail() {
  ...
  editAsset(detailAssetId);
}
function deleteAssetFromDetail() {
  closeModal('assetDetailModal');
  deleteAsset(detailAssetId);
}

// ---- Mise à jour rapide d'une valeur ----
function openQuickUpdate() {
  const a = state.wallet.find(x => x.id === detailAssetId);
  if (!a) return;
  document.getElementById('quickUpdateTitle').textContent = a.name;
  document.getElementById('quickUpdateOld').textContent = `Valeur actuelle : ${fmtMoney(a.value || 0)}`;
  document.getElementById('quickUpdateLabel').textContent = a.type === 'credit' ? 'Nouveau capital restant dû (€)' : (BUDGET_TYPES.includes(a.type) ? 'Nouveau montant mensuel (€)' : 'Nouvelle valeur (€)');
...
async function saveQuickUpdate(e) {
  e.preventDefault();
  const newValue = parseFloat(document.getElementById('quickUpdateValue').value);
  const note = document.getElementById('quickUpdateNote').value;
  if (isNaN(newValue)) return;

  await sbUpdate('wallet_assets', detailAssetId, { value: newValue });
  await sbInsert('asset_history', {
  ...
  toast('Valeur actualisée ✓', 'success');
  await loadAllData();
  renderCurrentView();
  openAssetDetail(detailAssetId);
}
...
function editAsset(id) {
  ...
  document.getElementById('assetDuration').value = a.duration_months || '';
  document.getElementById('assetRate').value = a.rate || '';
...
function deleteAsset(id) {
  deleteWithUndo('wallet_assets', id, 'Actif', () => {
...
// ==================== SETTINGS ====================
// (l'écran de configuration Supabase a été retiré : les identifiants sont désormais en dur dans le code, l'utilisateur n'a plus à s'en soucier)
...
function openProjectModal() {
  ...
  document.getElementById('projSubtypeFields').innerHTML = '';
...
function doSearch(q) {
  ...
  const qlow = normalizeSearch(q);
  const results = [];
  state.events.filter(e => normalizeSearch(e.title).includes(qlow)).forEach(e => results.push({ type:'Événement', title:e.title, meta:e.date + (e.time?' '+e.time:''), action:()=>{navigateTo('agenda');closeSearch();} }));
  state.tasks.filter(t => normalizeSearch(t.title).includes(qlow)).forEach(t => results.push({ type:'Tâche', title:t.title, meta:(t.due||'Sans échéance'), action:()=>{navigateTo('tasks');closeSearch();} }));
  state.projects.filter(p => normalizeSearch(p.title).includes(qlow)).forEach(p => results.push({ type:'Projet', title:p.title, meta:p.cat+' · '+p.phase, action:()=>{navigateTo('projects');closeSearch();} }));
  state.wallet.filter(a => normalizeSearch(a.name).includes(qlow)).forEach(a => results.push({ type:'Actif', title:a.name, meta:a.type, action:()=>{navigateTo('wallet');closeSearch();} }));
...
function toast(msg, type='info', actionLabel, actionFn) {
  ...
  const life = actionLabel ? 5000 : 4000;
  const timer = setTimeout(() => el.remove(), life);
  el._cancelTimer = () => clearTimeout(timer);
}

/**
 * Suppression avec possibilité d'annuler pendant 5s (pattern "undo" universel).
 * table: nom de la table Supabase ; id: identifiant de l'élément ;
 * label: nom affiché dans le toast ; onConfirmedDelete: fonction appelée si l'utilisateur n'annule pas.
 */
function deleteWithUndo(table, id, label, refreshFn) {
  let undone = false;
  const arrKey = table === 'wallet_assets' ? 'wallet' : table;
  const arr = state[arrKey];
  const idx = Array.isArray(arr) ? arr.findIndex(x => x.id === id) : -1;
  const removedItem = idx > -1 ? arr[idx] : null;
  if (idx > -1) arr.splice(idx, 1);
  renderCurrentView();
  if (refreshFn) refreshFn();

  toast(`${label} supprimé(e)`, 'info', 'Annuler', () => {
    undone = true;
    if (removedItem) { arr.splice(idx, 0, removedItem); renderCurrentView(); if (refreshFn) refreshFn(); }
    toast('Suppression annulée', 'success');
  });
  ...
  setTimeout(async () => {
    ...
    await loadAllData();
    if (refreshFn) refreshFn(); else renderCurrentView();
  }, 5000);
}
