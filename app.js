...
async function quickAddTask(e) {
  ...
  if (parsed.isEvent) {
    ...
    await sbInsert('tasks', {
      ...
      due: parsed.date ? parsed.date.toISOString().split('T')[0] : null,
      recurrence_rule: parsed.recurrence,
      notes: ''
    });
    toast(parsed.date ? `Tâche ajoutée pour le ${parsed.date.toLocaleDateString('fr-FR',{day:'numeric',month:'short'})} ✓` : 'Ajouté ✓', 'success');
  }
  input.value = '';
